...
class GitLabClient:
    ...
    def __getattribute__(self, name: str):
        ...
        if (
            name.startswith(("get_", "post_", "create_", "update_", "list_", "set_", "add_"))
            and callable(attr)
            and not name.startswith("_")
        ):
            def instrumented(*args, **kwargs):
                telemetry = object.__getattribute__(self, "telemetry")
                trace_id = object.__getattribute__(self, "trace_id")
                telemetry.emit(
                    ...
                    destination="gitlab",
                    tool_name=name,
                    metadata={"arg_count": len(args) + len(kwargs)},
                )
    ...
    def post_mr_comment(self, mr_iid: int, body: str) -> None:
        ...
        """
        try:
            protected_body = self.protection.check_output(
                body,
                {"boundary": "pre_output", "destination": "gitlab_mr", "trace_id": self.trace_id},
            )
            mr = self._project.mergerequests.get(mr_iid)
            mr.notes.create({"body": protected_body})
    ...
    def post_issue_comment(self, issue_iid: int, body: str) -> None:
        ...
        try:
            protected_body = self.protection.check_output(
                body,
                {"boundary": "pre_output", "destination": "gitlab_issue", "trace_id": self.trace_id},
            )
            issue = self._project.issues.get(issue_iid)
            issue.notes.create({"body": protected_body})
        except gitlab_exc.GitlabCreateError as e:
    ...
    def create_issue(self, title: str, description: str, labels: list[str] | None = None) -> dict:
        ...
        """
        try:
            protected_description = self.protection.check_output(
                description,
                {"boundary": "pre_output", "destination": "gitlab_issue", "trace_id": self.trace_id},
            )
            data: dict[str, Any] = {"title": title, "description": protected_description}
    ...
    def create_wiki_page(self, slug: str, title: str, content: str) -> None:
        ...
        """
        try:
            protected_content = self.protection.check_output(
                content,
                {"boundary": "pre_output", "destination": "gitlab_wiki", "trace_id": self.trace_id},
            )
            self._project.wikis.create({"title": slug, "content": protected_content})
        except gitlab_exc.GitlabCreateError as e:
    ...
    def update_wiki_page(self, slug: str, content: str) -> None:
        ...
        try:
            page = self._project.wikis.get(slug)
            page.content = self.protection.check_output(
                content,
                {"boundary": "pre_output", "destination": "gitlab_wiki", "trace_id": self.trace_id},
            )
            page.save()
