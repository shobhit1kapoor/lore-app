    ...
    <AppShell>
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Every agent, tool, LLM, memory, and protection event grouped by trace ID.</p>
        </div>
            ...
            <a key={trace.trace_id} href={`/traces/${trace.trace_id}`} className="grid gap-4 border-b border-line p-4 last:border-b-0 hover:bg-panel lg:grid-cols-[1fr_160px_120px]">
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {trace.event_types.slice(0, 6).map((type) => <span key={type} className="bg-panel px-2 py-1 text-xs text-slate-700">{type}</span>)}
                </div>
              </div>
              <div className="text-sm text-slate-600">{trace.event_count} events</div>
              <RiskBadge blocked={trace.blocked} score={trace.max_risk_score} />
            </a>
          )) : <div className="p-6 text-sm text-slate-500">No traces recorded yet.</div>}
        </div>
      </div>
    </AppShell>
  );
